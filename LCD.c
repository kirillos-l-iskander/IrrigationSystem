...
static uint8_t LCD_Buff_G[ROW_NUM][COL_NUM];
...
void LCD_Init(void)
{
	...
	LCD_ClearBuffer();
	GPIO_InitPin(LCD_COM_REG, RS, OUTPUT);
	GPIO_InitPin(LCD_COM_REG, E, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D0, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D1, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D2, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D3, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D4, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D5, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D6, OUTPUT);
	GPIO_InitPin(LCD_DAT_REG, D7, OUTPUT);
...
{
	GPIO_SetPortState(LCD_DAT_REG, DPINS, Char);
